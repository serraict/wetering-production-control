...
1. **Review**: Click "Openstaande wijzigingen" button to review pending changes
1. **Commit**: Click "Opslaan in database" to persist changes to Firebird
1. **Confirmation**: Success notification confirms database update
1. **Clear**: Pending changes automatically cleared on successful commit
...
1. **Security**: Consider using secure credentials storage (e.g., secrets manager)
1. **Error Handling**: Monitor API logs for any connection or update errors
1. **Testing**: Verify with Bianca that the workflow works as expected
1. **Rollback Plan**: Keep the manual desktop app workflow as fallback during initial deployment
